...
ax2.plot(20, 46, 'bx')
ax2.plot(79, 79, 'bx')
ax2.title.set_text('Acquisition Function' if type == 'acq-func' else 'World Model')
xs = np.array([])
ys = np.array([])
cbar, pos1, marker1, pos2, marker2, im, lab = None, None, None, None, None, None, None
...
def update(frame):
    ...
    if cbar is not None:
        cbar.remove()
    if type == 'acq-func':
        world_model = acq_func.utility(np.vstack([X.ravel(), Y.ravel()]).transpose(), optimizer._gp, 0)
    else:
        world_model = optimizer._gp.predict(np.vstack([X.ravel(), Y.ravel()]).transpose())
    world_model = world_model.reshape(X.shape)
    if marker2 is not None:
        ax2.lines.pop()
    if type == 'acq-func':
        im = ax2.imshow(world_model, cmap='hot', interpolation='nearest')
        im.set_clim(vmin=0)
        ...
        if im is not None:
            ...
            for coll in im.collections:
                coll.remove()
        im = ax2.contour(X, Y, world_model, levels=12, cmap='Blues', vmin=CS.zmin, vmax=CS.zmax)
        pos = im.ax.get_position(original=True)
        lab = plt.clabel(im, inline=1, fontsize=8, fmt='%.3g')
    if frame == 0 or not reset_loc:
    ...
    t.update(n=1)
    plt.tight_layout()
    return marker1, marker2, pos1, pos2, ax2
...
if debug:
    ...
    anim = animation.FuncAnimation(fig, update, save_count=len(points), frames=len(points)-1, blit=True if type == 'acq-func' else False)
